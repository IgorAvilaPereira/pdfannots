...
def parse_args_igor() -> typ.Tuple[argparse.Namespace, LAParams]:
    p = argparse.ArgumentParser(prog='pdfannots', description=__doc__)
...
def main_igor(file_igor: str)-> None:
    args, laparams = parse_args_igor()
    logging.basicConfig(format='%(levelname)s: %(message)s',
                        level=logging.WARNING)
    ...
    write_if_nonempty(printer.end())
    # return printer.print_file(file.name, doc)
